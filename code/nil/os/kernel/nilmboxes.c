 ...
 *          A mailbox is an asynchlronous communication mechanism.<br>
 ...
 *          by the message.
 * @pre     In order to use the mailboxes APIs the @p NIL_USE_MAILBOXES option
 *          must be enabled in @p nilconf.h.
 * @{
 */
...
//  nilDbgCheck((mbp != NULL) && (buf != NULL) && (n > 0), "nilMBInit");

  mbp->mb_buffer = mbp->mb_wrptr = mbp->mb_rdptr = buf;
  mbp->mb_top = &buf[n];
  nilSemInit(&mbp->mb_emptysem, n);
  nilSemInit(&mbp->mb_fullsem, 0);
}
...
//  nilDbgCheck(mbp != NULL, "nilMBReset");

  nilSysLock();
  mbp->mb_wrptr = mbp->mb_rdptr = mbp->mb_buffer;
  nilSemResetI(&mbp->mb_emptysem, mbp->mb_top - mbp->mb_buffer);
  nilSemResetI(&mbp->mb_fullsem, 0);
  nilSchRescheduleS();
  nilSysUnlock();
}
 ...
 * @api
 */
msg_t nilMBPost(Mailbox *mbp, msg_t msg, systime_t time) {
  msg_t rdymsg;

  nilSysLock();
  rdymsg = nilMBPostS(mbp, msg, time);
  nilSysUnlock();
  return rdymsg;
}
 ...
 * @sclass
 */
msg_t nilMBPostS(Mailbox *mbp, msg_t msg, systime_t time) {
  msg_t rdymsg;

//  nilDbgCheckClassS();
//  nilDbgCheck(mbp != NULL, "nilMBPostS");

  rdymsg = nilSemWaitTimeoutS(&mbp->mb_emptysem, time);
  if (rdymsg == RDY_OK) {
    *mbp->mb_wrptr++ = msg;
    if (mbp->mb_wrptr >= mbp->mb_top)
      mbp->mb_wrptr = mbp->mb_buffer;
    nilSemSignalI(&mbp->mb_fullsem);
    nilSchRescheduleS();
  }
  return rdymsg;
}
...
//  nilDbgCheckClassI();
//  nilDbgCheck(mbp != NULL, "nilMBPostI");

//  if (nilSemGetCounterI(&mbp->mb_emptysem) <= 0)
//    return RDY_TIMEOUT;
  nilSemWait(&mbp->mb_emptysem);
  *mbp->mb_wrptr++ = msg;
  if (mbp->mb_wrptr >= mbp->mb_top)
    mbp->mb_wrptr = mbp->mb_buffer;
  nilSemSignalI(&mbp->mb_fullsem);
 ...
 * @api
 */
msg_t nilMBPostAhead(Mailbox *mbp, msg_t msg, systime_t time) {
  msg_t rdymsg;

  nilSysLock();
  rdymsg = nilMBPostAheadS(mbp, msg, time);
  nilSysUnlock();
  return rdymsg;
}
 ...
 * @sclass
 */
msg_t nilMBPostAheadS(Mailbox *mbp, msg_t msg, systime_t time) {
  msg_t rdymsg;

//  nilDbgCheckClassS();
//  nilDbgCheck(mbp != NULL, "nilMBPostAheadS");
...
//  nilDbgCheckClassI();
//  nilDbgCheck(mbp != NULL, "nilMBPostAheadI");
 ...
 * @api
 */
msg_t nilMBFetch(Mailbox *mbp, msg_t *msgp, systime_t time) {
  msg_t rdymsg;

  nilSysLock();
  rdymsg = nilMBFetchS(mbp, msgp, time);
  nilSysUnlock();
  return rdymsg;
}
 ...
 * @sclass
 */
msg_t nilMBFetchS(Mailbox *mbp, msg_t *msgp, systime_t time) {
  msg_t rdymsg;

//  nilDbgCheckClassS();
//  nilDbgCheck((mbp != NULL) && (msgp != NULL), "nilMBFetchS");

  rdymsg = nilSemWaitTimeoutS(&mbp->mb_fullsem, time);
  if (rdymsg == RDY_OK) {
    *msgp = *mbp->mb_rdptr++;
    if (mbp->mb_rdptr >= mbp->mb_top)
      mbp->mb_rdptr = mbp->mb_buffer;
    nilSemSignalI(&mbp->mb_emptysem);
    nilSchRescheduleS();
  }
  return rdymsg;
}
...
//  nilDbgCheckClassI();
//  nilDbgCheck((mbp != NULL) && (msgp != NULL), "nilMBFetchI");

//  if (nilSemGetCounterI(&mbp->mb_fullsem) <= 0)
//    return RDY_TIMEOUT;
  nilSemWait(&mbp->mb_fullsem);
  *msgp = *mbp->mb_rdptr++;
  if (mbp->mb_rdptr >= mbp->mb_top)
    mbp->mb_rdptr = mbp->mb_buffer;
  nilSemSignalI(&mbp->mb_emptysem);
